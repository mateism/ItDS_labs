...
```{r, echo= FALSE}
frml_str_1 <- paste("life_expectancy", "~ gdp + population + alcohol")
frml_str_2 <- paste("life_expectancy", "~ gdp + population + alcohol + gdp * population")
frml_str_3 <- paste("life_expectancy", "~ gdp + I(gdp^2) + log(population)")

frml_1 <- as.formula(frml_str_1)
frml_2 <- as.formula(frml_str_2)
frml_3 <- as.formula(frml_str_3)

reg1 <- lm(frml_1, data = life_expec_dat)
reg2 <- lm(frml_2, data = life_expec_dat)
reg3 <- lm(frml_3, data = life_expec_dat)
...
```{r, echo= FALSE}
mod_table2 <- modelsummary::modelsummary(multiv_model_out[29:31], 
                                        output = "kableExtra",
                                        fmt = fmt_significant(2),  # 2 significant digits
                                        estimate  = "{estimate}",
                                        statistic = "conf.int",
                                        coef_omit = "Intercept",
                                        coef_rename=c("gdp"="Gdp", 
                                                      ...
                                                      "life_expectancy" = "Life Expectancy"),
                                        gof_omit = 'DF|Deviance|Log.Lik|AIC|BIC',
                                        notes = "Data source: WHO (https://www.kaggle.com/kumarajarshi/life-expectancy-who)",
                                        title = 'An Improved Regression Table',
                                        escape = FALSE)
