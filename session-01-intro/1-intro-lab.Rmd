...
```{r,, eval=F}
dplyr::select(penguins, species, island, body_mass_g, sex, rec_year)
...
```{r , eval=F}
dplyr::filter(penguins, rec_year == 2007)
...
final_df <- penguins |> 
  filter(rec_year == 2009, species == "Chinstrap") |>
  select(species, sex, rec_year)
final_df
```
...
-   when using `.by` you **do not need** to ungroup afterwards. [See here](https://dplyr.tidyverse.org/reference/dplyr_by.html)

-   to remove `NA` values from the `sex` column below either drop them first or filter them out after summarizing

```{r}
#calls for ungroup() - and preceding pipes - commented out below since using summarize with .by doesn't require it (unlike groupby())
...
penguins |>
  ...
  #Marci's addition to filter NAs from the sex column:
  filter(!is.na(sex)) #|>
  #Original code cont.:
  # dplyr::ungroup()
```

> Notice that we are using `dplyr::ungroup()` after performing grouped calculations. It is a convention we encourage. If you forget to `ungroup()` data, future data management can produce errors in downstream operations. Just to be safe, use `dplyr::ungroup()` when you've finished with your calculations.

::: alert-info
**Exercise 3**

Can you get the weight of the lightest penguin of each species? You can use `min()`. What happens when in addition to species you also group by year `group_by(species, rec_year)`?
:::

```{r,, eval=F}
penguins |> 
  group_by(species) |>
  summarise(lightest_weight = min(body_mass_g, na.rm = TRUE)) |>
  ungroup()

penguins |> 
  summarise(lightest_weight = min(body_mass_g, na.rm = TRUE), .by = c(species, rec_year)) |>
  # ungroup()
```
...
```{r,, eval=F}
dream_weight_desc_df <- penguins |>
  filter(island == "Dream") |>
  arrange(-body_mass_g)
dream_weight_desc_df
```
